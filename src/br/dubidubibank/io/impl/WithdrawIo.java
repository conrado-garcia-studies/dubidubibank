...
public class WithdrawIo extends ScreenTemplateIo {
    private final CommandDao commandDao;
    private final TransactionDao transactionDao;

    public WithdrawIo(CommandDao commandDao, SessionService sessionService, TransactionDao transactionDao) {
    ...
    protected void executeInternal(Scanner scanner) {
        ...
        if (value < 0) {
            throw new IllegalArgumentException("You cannot withdraw a negative value.");
        }
        Optional<Command> command = commandDao.get(CommandCode.WITHDRAW);
        if (command.isEmpty()) {
        ...
        if (todaysTransaction.isPresent()) {
            throw new IllegalArgumentException("You cannot withdraw more than once a day. Please try again tomorrow.");
        }
